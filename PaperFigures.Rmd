...
one<-allplot(melt, "TB", "value", all.cols)+    
    facet_wrap(~variable, scales="free", labeller = cyt_groups, ncol=2)+
    labs(y="(%) Total CD4+ T cells")+
    theme(legend.position="none") 

melt<-melt(cyt, id.vars=c("TB","SM", "disease"), measure.vars=c("IL4"))

four<-allplot(melt, "TB", "value", all.cols)+    
    facet_wrap(~variable, scales="free", labeller = cyt_groups, ncol=1)+
    theme(legend.position="none")+
    coord_cartesian(ylim=c(0,7))

melt<-melt(cyt, id.vars=c("TB","SM", "disease"), measure.vars=c("IL13"))


thirteen<-allplot(melt, "TB", "value", all.cols)+    
    facet_wrap(~variable, scales="free", labeller = cyt_groups, ncol=1)+
    theme(legend.position="none")+
    coord_cartesian(ylim=c(0,3))

cowplot::plot_grid(one, four, thirteen, ncol=3, rel_widths=c(7.5,3.75, 3.75))
```
...
a<-baseplot(melt[melt$variable=="IFNg",], "SM", "value", ltbi.cols)+    
    facet_wrap(~variable, labeller = cyt_groups)+
    stat_compare_means(label="p.format", size=6, label.x=1.3, label.y = (.95*.15))+
    coord_cartesian(ylim=c(0,.15))+
    labs(y="(%) CD4+ T cells")

b<-baseplot(melt[melt$variable=="TNFa",], "SM", "value", ltbi.cols)+    
    facet_wrap(~variable, labeller = cyt_groups)+
    stat_compare_means(label="p.format", size=6, label.x=1.3, label.y = (.95*.15))+
    coord_cartesian(ylim=c(0,.15))

c<-baseplot(melt[melt$variable=="IL4",], "SM", "value", ltbi.cols)+    
    facet_wrap(~variable, labeller = cyt_groups)+
    stat_compare_means(label="p.format", size=6, label.x=1.3, label.y = (.95*.013))+
    coord_cartesian(ylim=c(0,.013))

d<-baseplot(melt[melt$variable=="IL13",], "SM", "value", ltbi.cols)+    
    facet_wrap(~variable, labeller = cyt_groups)+
