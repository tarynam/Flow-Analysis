...
second_row = cowplot::plot_grid(tf, sm,labels = c('B', 'C'), nrow=1, label_size = 20)
...
tf<-allplot(melt, "TB", "value", all.cols)+    
    labs(y="(%) Total CD4+ T cells")+
    scale_x_discrete(labels=c("SM- SM+\nHC", "SM- SM+\nLTBI", "SM- SM+\nTB"))+
    stat_compare_means(method = 'wilcox.test', label="p.format", size=6, label.y=.9*30)+
    coord_cartesian(ylim=c(0,30))
...
sm<-allplot(melt, "TB", "value", all.cols) +    
    labs(y="(%) Total CD4+ T cells")+
    scale_x_discrete(labels=c("SM- SM+\nHC", "SM- SM+\nLTBI", "SM- SM+\nTB"))+
    stat_compare_means(method = 'wilcox.test', label="p.format", size=6, label.y=.9*65)+
...
figure1B<- cowplot::plot_grid(first_row, second_row, ncol=1, rel_heights = c(1,2))
```
...
sm<-allplot(melt, "disease", "value", all.cols) +    
    ...
    scale_x_discrete(labels=c("HC\nSM-", "HC\nSM+", 
                              "LTBI\nSM-", "LTBI\nSM+", 
                              "TB\nSM-", "TB\nSM+"))+
    stat_compare_means(comparisons=list(c("TB SM-", "TB SM+")),method = 'wilcox.test', label="p.format", size=6, label.y=.9*65)+
    coord_cartesian(ylim=c(0,65))
...
first_row = cowplot::plot_grid(NULL, labels = c('A'), nrow=1, label_size = 20, rel_widths = c(3,2))
second_row = cowplot::plot_grid(tf, sm,labels = c('B', 'C'), nrow=1, label_size = 20)
figure1C<- cowplot::plot_grid(first_row, second_row, ncol=1, rel_heights = c(1,2))
```
...
second_row = cowplot::plot_grid(one, two, labels = c('B', 'C'), nrow=1, label_size = 20)
third_row = cowplot::plot_grid(a, b, c, nrow=1, labels=c('D'), label_size = 20)
...
first_row = cowplot::plot_grid(NULL, labels = c('A'), nrow=1, label_size = 20)
second_row = cowplot::plot_grid(a, b, tf, sm, nrow=1, labels=c('B', 'C', 'D', 'E'), label_size = 20)
...
pep<-allplot(part2, "TB", "value", all.cols)+
    ...
    labs(title="CFP-10 & ESAT-6")


first_row = cowplot::plot_grid(NULL, labels = c('A'), nrow=1, label_size = 20)
second_row = cowplot::plot_grid(seb, pep, nrow=1, rel_widths = c(3,2), labels=c("B", "C"), label_size = 20)
figure6<-cowplot::plot_grid(first_row, second_row, ncol=1, rel_heights = c(1.5,3))
...
first_row = cowplot::plot_grid(NULL, labels = c('A'), nrow=1, label_size = 20)
second_row = cowplot::plot_grid(one, two, nrow=1, rel_widths = c(2,1), labels=c("B", "C","D"), label_size = 20)
...
c<-baseplot(melt, "SM", "value", ltbi.cols)+facet_wrap(~variable, scales="free")+
    labs(y="pg/mL")+
    coord_cartesian(ylim=c(0,20))

lumplot<-cowplot::plot_grid(a,b,c, NULL, ncol=1, align = "v", labels=c("A","A", "A", "B"), label_size = 20)
```
...
cowplot::save_plot("figure1alt2.png", figure1C, base_height=8, base_width=16) #Figure 1
...
cowplot::save_plot("figure4-1.png", figure4opt1, base_height=10, base_width=16)
cowplot::save_plot("figure4-2.png", figure4opt2, base_height=15, base_width=14)

cowplot::save_plot("figure5.png", figure5, base_height=11, base_width=16)
cowplot::save_plot("figure6.png", figure6, base_height=10, base_width=12)
cowplot::save_plot("figure7.png", figure7, base_height=16, base_width=12)
cowplot::save_plot("figure8.png", lumplot, base_height=48, base_width=16)
```
...
second_row = cowplot::plot_grid(og, one, two, nrow=1, rel_widths = c(1,2,1), labels=c("B","C","D"), label_size = 20)
