...
pval<-c(A$`SM-.HC`$p.value, A$`SM+.HC`$p.value,
        A$`SM-.LTBI`$p.value, A$`SM+.LTBI`$p.value, 
        A$`SM-.TB`$p.value, A$`SM+.TB`$p.value)
group<-c("HC SM-", "HC SM+", "LTBI SM-", "LTBI SM+", "TB SM-", "TB SM+")
table<-data.frame(cbind(group, pval))
table$pval<-as.numeric(as.character(table$pval))
table$pval[table$pval<.0001]<-"<0.0001"
kable(table, caption="TH1 to TH2")
```

some stats comparing th1 to th1/2 for reference. 
```{r}
...
table$pval<-as.numeric(as.character(table$pval))
table$pval[table$pval<.0001]<-"<0.0001"
kable(table, caption="TH1 to TH1/2")
```
...
one<-baseplot(melt, "disease", "value", ltbi.cols)+    
...
two<-baseplot(melt, "disease", "value", ltbi.cols)+    
    facet_wrap(~variable, labeller = groups)
```
