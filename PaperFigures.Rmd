...
#All groups
Dont worry I can change this so the ns bars don't show up, especially for the cytokines since I have no idea why that giant NS is there. But for now I'm leaving them so we know.
```{r}
cd4<-filter_all(subset(CD4, CD4$Stim=="UN"))
cyt<-filter_all(subset(CYT, CYT$Stim=="PMA"))
phen<-filter_all(subset(PHEN, PHEN$Stim=="PMA"))
og<-filter_all(subset(OG, OG$Stim=="SEB"))
ogphen<-filter_all(subset(OGPHEN, OGPHEN$Stim=="SEB"))
```
...
allplot(melt, "TB", "value", all.cols)+    
    facet_wrap(~variable, scales="free", labeller = cyt_groups, ncol=4)+
    labs(y="(%) Total CD4+ T cells")+
    theme(legend.position="bottom") 
```
...
```{r cytokine, fig.width=16, fig.height=8}
melt<-melt(cyt, id.vars=c("TB","SM", "disease"), measure.vars=c("IFNg", "TNFa", "IL4", "IL13"))
...
```{r ics main four markers, fig.width=16, fig.height=8}
...
```{r main four markers, fig.width=16, fig.height=8}
...
```{r og and cyt, fig.width=16, fig.height=8}
...
cowplot::plot_grid(a, b, ncol=2, rel_widths=c(1,3.5), align="h")
```
