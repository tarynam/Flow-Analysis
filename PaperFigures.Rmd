...
a<-baseplot(melt, "SM", "value", ltbi.cols)+    
    ...
    stat_compare_means(method="wilcox.test" , label="p.format", size=6, label.x=1.3, label.y = (.95*.15))+
...
b<-baseplot(melt, "SM", "value", ltbi.cols)+    
    facet_wrap(~variable, labeller = cyt_groups)+
    stat_compare_means(method="wilcox.test" , label="p.format", size=6, label.x=1.3, label.y = (.95*.02))+
...
a<-baseplot(melt[melt$variable %in% c("T-bet","GATA3"),], "SM", "value", ltbi.cols)+    
    facet_wrap(~variable)+
    stat_compare_means(method="wilcox.test" , label="p.format", size=6, label.x=1.3, label.y=(.98*70))+
    coord_cartesian(ylim=c(0,70))+
    labs(y="(%) Mtb-Specific TH1 Cytokine+ cells")

b<-baseplot(melt[melt$variable %in% c("CXCR3", "CCR4"),], "SM", "value", ltbi.cols)+    
    facet_wrap(~variable)+
    stat_compare_means(method="wilcox.test" , label="p.format", size=6, label.x=1.3, label.y=(.98*100))+
...
a<-baseplot(melt[melt$variable %in% c("T-bet","GATA3"),], "SM", "value", tb.cols)+    
    facet_wrap(~variable)+
    stat_compare_means(method="wilcox.test" , label="p.format", size=6, label.y=(.98*80))+
...
baseplot(melt, "SM", "value", tb.cols)+    
    ...
    stat_compare_means(method="wilcox.test" , label="p.format", size=6, label.x=1.3)+
    labs(y="(%) Mtb-Specific TH1 Cytokine+ cells")
```
...
groups <- as_labeller(c("OG_freq" = "Proliferation","IFNg_freq" = "IFNγ", 
                    "TNFa_freq" = "TNFα", "IL4_freq" = "IL-4"))

part1<-melt(og, id.vars=c("TB","SM", "disease"), measure.vars="OG_freq")
og<-allplot(part1, "TB", "value", all.cols)+
    facet_wrap(~variable, scales="free", labeller = groups)+
    labs(y="(%) Total CD4+ T cells")

melt<-melt(ogphen, id.vars=c("TB","SM", "disease"), measure.vars=c("IFNg_freq", "TNFa_freq"))

one<-allplot(melt, "TB", "value", all.cols)+    
    facet_wrap(~variable, labeller = groups, ncol=2)+
    labs(y="(%) Proliferating CD4+ T cells")

melt<-melt(ogphen, id.vars=c("TB","SM", "disease"), measure.vars=c("IL4_freq"))

two<-allplot(melt, "TB", "value", all.cols)+    
    facet_wrap(~variable, labeller = groups)

cowplot::plot_grid(og, one, two, ncol=3, rel_widths = c(1.2,2,1))
```
...
melt<-melt(og, id.vars=c("TB","SM", "disease"), measure.vars="OG_freq")
...
cowplot::plot_grid(th1.2, th1, th2, ncol=3, rel_widths = c(8,3.5,3.5))
```
...
booleanplot2(test, "variable", "value", all.cols)+
...
cowplot::plot_grid(b, c, ncol=3)
```

```{r, fig.width=16, fig.height=8}
cyt<-plot_filter(CYT[CYT$Stim=="PEP",])
cyt<-filter(cyt, TB!="HC")
test<-melt(cyt, id.vars=c("TB", "SM", "disease"), measure.vars = grep("._._._.|._._.._.", names(cyt)))
test<-test[-grep("tbet|gata3|x_x_x_x", test$variable),]
test$variable<-factor(test$variable, levels=c(
"G_4_13_T", "G_4_x_T", "G_x_13_T", "G_4_13_x", "x_4_13_T","G_4_x_x", "G_x_13_x","x_4_x_T","x_x_13_T",
...
booleanplot2(test, "variable", "value", all.cols)+facet_grid(TB~th, scales="free", space="free")+
    scale_x_discrete(labels = labels, breaks=breaks)+
