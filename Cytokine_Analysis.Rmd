...
CD4b<-plot_filter(read.csv("ICS_Cytokine_new.csv"))
...
colnames<-c("TH1_total", "TH2_total")
for(col in colnames){#this will loop through each character string in the list
    a<-subset(CD4b, CD4b$Stim=="WCL") #subsets the data to only contain the unstimulated samples
    #inserts each character string into the column argument of the pvals function written above
    b<-pvals_4(a, col)
    if(dim(b)[1]!=0)
    print(kable(b, caption=col)) #will return the output for each character string in the loop
...
colnames<-c("TH1_total", "TH2_total")
for(col in colnames){#this will loop through each character string in the list
    a<-subset(CD4b, CD4b$Stim=="PEP") #subsets the data to only contain the unstimulated samples
    #inserts each character string into the column argument of the pvals function written above
    b<-pvals_4(a, col)
    if(dim(b)[1]!=0)
    print(kable(b, caption=col)) #will return the output for each character string in the loop
...
for(col in colnames){#this will loop through each character string in the list
    #inserts each character string into the column argument of the pvals function written above
    name<-gsub("_", "+", col)
    y<-plot_with_all_stats(CD4b, "PMA", col)+
        labs(title=paste("Frequency of ", name, "+ CD4+ T cells"), 
             title="PMA & Ionomycin",
             x="",
             y=paste(col, "+ CD4+ T cells (%)"))
